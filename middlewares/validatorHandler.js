...
function validatorHandler (schema, property){                         //este middleware es dinámico, ya que recibe el esquema a validar y las propiedades
  return (req, resp, next) => {                                       //esta línea es un closure, ya que retorna una función con formato middleware básico, con request, response y next
    ...
    const { error } = schema.validate(data, { abortEarly: false });   //dinámico [property] => esto es así para no tener que especificar si viene de un req.body, req.params, o req.query
...
module.exports = validatorHandler;                //esto se debe exportar a las rutas, ya que ahí están los request's y response's
